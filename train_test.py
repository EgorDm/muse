...
def run(settings):
    ...
    model = KerasEmbeddingModel(batcher, settings.nlayers, settings.cell_size, 1.0 - settings.kprob, settings.lr)

    trainer = KerasTrainer(batcher, validation_batcher, model, settings)
